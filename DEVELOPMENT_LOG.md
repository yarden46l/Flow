...
### [2026-06-21] Feature & Bug Fix: Rescheduling, Fixed Blocks, Task Completion, Smart Suggest
- **Fix — Calendar Rescheduling (`src/app/page.tsx` Case 2):** Moving an already-scheduled task to a new time slot now includes an optimistic `setTasks(prev.map(...))` immediately before `updateTask`, so the task visually jumps to the new slot without waiting for Firestore.
- **Feature — Fixed Blocks (`src/app/page.tsx` + `src/components/CaptureZone.tsx`):** Added `handleAddFixedBlock(title, startTime, endTime)` handler that creates a `Task` with `isFixedAnchor: true`, `type: "fixed"`, `status: "scheduled"` and optimistically adds it to state + Firestore. Added `onAddFixedBlock` prop and a "Fixed Block (Anchor)" toggle-button/form to `CaptureZone` with title, start time, and end time inputs and validation. Fixed blocks appear on the calendar as dashed locked cards (already styled by `DraggableEvent`) and are respected by Smart Suggest as occupied time.
- **Fix — Task Completion (`src/components/MicroExecutionPanel.tsx` + `src/app/page.tsx`):** The ✓ button in the Pomodoro timer now calls `onCompleteTask(taskId)` (a new prop) instead of calling `updateTask` directly (which had no effect on React state). `handleCompleteTask` in `page.tsx` immediately sets `status: "archived"`, `isCompleted: true`, `completedAt` in local state and clears `activeEventId` so the panel returns to empty state. Firestore is updated in the background.
- **Fix — Smart Suggest algorithm (`src/app/page.tsx`):** (1) Replaced 30-min hardcoded slot array with a 15-min granularity numeric loop (06:00–22:00) for tighter packing. (2) Added a two-pass strategy: pass 0 = strict energy/friction routing, pass 1 = relaxed fallback — prevents tasks from being permanently unschedulable when the ideal window is full. (3) Fixed the occupied-minutes calculation to use `tasks` state directly (not just `weekdayEvents`) so optimistically-applied drags and previous Smart Suggest runs are respected. (4) Moved the weekend guard to before the async work to prevent a stale spinner.
